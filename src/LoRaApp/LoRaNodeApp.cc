...
#define TIME_ON_AIR_SF_CAD_SF 12
    ...
    void LoRaNodeApp::initialize(int stage)
    {
        ...
        if (routingMetric == 0)
        { // for the END nodes, where no forwarding
        ...
        if (stage == INITSTAGE_LOCAL)
        {
            ...
            else if (strcmp(host->par("deploymentType").stringValue(), "grid") == 0)
            {
                ...
                StationaryMobility *mobility = check_and_cast<StationaryMobility *>(
                    host->getSubmodule("mobility"));
                if (nodeId == 0 && routingMetric == 0)
                { // end node 0 at middle
                    mobility->par("initialX").setDoubleValue(minX + sepX * (cols / 2));
                    mobility->par("initialY").setDoubleValue(minY + sepY * (cols / 2) + uniform(0, 100));
                }
                else
                {
                    mobility->par("initialX").setDoubleValue(minX + sepX * (nodeId % cols) + uniform(0, 100));
                    mobility->par("initialY").setDoubleValue(minY + sepY * ((int)nodeId / cols) + uniform(0, 100));
                }
            }
            else
            {
        ...
        else if (stage == INITSTAGE_APPLICATION_LAYER)
        {
            ...
            exportRoutingTable = par("exportRoutingTable");
            logRouteSelection = par("logRouteSelection");
            forceDestination = par("forceDestination");
            enableDataGeneration = par("enableDataGeneration");
    ...
    void LoRaNodeApp::finish()
    {
        ...
        for (std::vector<LoRaAppPacket>::iterator lbptr =
                 LoRaPacketsToForward.begin();
             lbptr < LoRaPacketsToForward.end();
             lbptr++)
        {
        ...
        for (std::vector<LoRaAppPacket>::iterator lbptr =
                 LoRaPacketsForwarded.begin();
             lbptr < LoRaPacketsForwarded.end();
             lbptr++)
        {
        ...
        for (std::vector<LoRaAppPacket>::iterator lbptr =
                 DataPacketsForMe.begin();
             lbptr < DataPacketsForMe.end();
             lbptr++)
        {
        ...
        // Optionally export routing table(s) for post-processing
        if (exportRoutingTable)
        {
            std::ostringstream fname;
            fname << "routingTable-node" << nodeId << ".txt";
            std::ofstream out(fname.str().c_str(), std::ios::out | std::ios::trunc);
            if (out.is_open())
            {
                out << "# Node " << nodeId << " routingMetric=" << routingMetric << " time=" << simTime() << "\n";
                if (singleMetricRoutingTable.size() > 0)
                {
                    out << "# singleMetricRoutingTable: id,via,metric,validUntil\n";
                    for (auto &r : singleMetricRoutingTable)
                    {
                        out << r.id << "," << r.via << "," << r.metric << "," << r.valid << "\n";
                    }
                }
                if (dualMetricRoutingTable.size() > 0)
                {
                    out << "# dualMetricRoutingTable: id,via,sf,priMetric,secMetric,validUntil\n";
                    for (auto &r : dualMetricRoutingTable)
                    {
                        out << r.id << "," << r.via << "," << r.sf << "," << r.priMetric << "," << r.secMetric << "," << r.valid << "\n";
                    }
                }
                out.close();
            }
    ...
    void LoRaNodeApp::logChosenRoute(const char *phase, int dest, int via, bool broadcastFallback)
    {
        if (!logRouteSelection)
            return;
        std::ostringstream fname;
        fname << "routeUse-node" << nodeId << ".log";
        std::ofstream out(fname.str().c_str(), std::ios::out | std::ios::app);
        if (out.is_open())
        {
            out << simTime() << "," << phase << ",dest=" << dest << ",via=" << via
                << ",broadcast=" << (broadcastFallback ? 1 : 0) << std::endl;
        }
    }
        ...
        if (lrmc->fsm.getState() == IDLE)
        {
            ...
            if (!sendPacketsContinuously && routingPacketsDue)
            {
                ...
                for (int i = 0; i < numberOfNodes; i++)
                {
                    ...
                    if (!(lrndpp->lastDataPacketTransmissionTime > 0 && // ToDo: maybe too restrictive? If no packets were received at all
                ...
                for (int i = 0; i < packet->getRoutingTableArraySize(); i++)
                {
                    ...
                    if (thisRoute.getId() != nodeId)
                    {
                        ...
                        if (!isRouteInSingleMetricRoutingTable(thisRoute.getId(), packet->getSource()))
                        {
                            ...
                            case ETX_SINGLE_SF:
                                newRoute.metric =
                                    singleMetricRoutingTable[getRouteIndexInSingleMetricRoutingTable(packet->getSource(), packet->getSource())].metric + thisRoute.getPriMetric();
                                break;
                            }
    ...
    simtime_t LoRaNodeApp::sendDataPacket()
    {
        ...
        if (
            (LoRaPacketsToSend.size() > 0 && bernoulli(ownDataPriority)) || (LoRaPacketsToSend.size() > 0 && LoRaPacketsToForward.size() == 0))
        {
        ...
        if (transmit)
        {
            ...
            case FLOODING_BROADCAST_SINGLE_SF:
                ...
                logChosenRoute("tx", dataPacket->getDestination(), BROADCAST_ADDRESS, true);
                break;
            case SMART_BROADCAST_SINGLE_SF:
            case HOP_COUNT_SINGLE_SF:
            case RSSI_SUM_SINGLE_SF:
            ...
            case ETX_SINGLE_SF:
                if (routeIndex >= 0)
                {
                    ...
                    logChosenRoute("tx", dataPacket->getDestination(), dataPacket->getVia(), false);
                }
                else
                {
                    dataPacket->setVia(BROADCAST_ADDRESS);
    ...
    void LoRaNodeApp::generateDataPackets()
    {
        ...
        if (enableDataGeneration && (!onlyNode0SendsPackets || nodeId == 0))
        {
            std::vector<int> destinations = {};

            if (forceDestination >= 0 && forceDestination != nodeId)
            {
                destinations.push_back(forceDestination);
            }
            else
            {
                if (numberOfDestinationsPerNode == 0)
        ...
        bool LoRaNodeApp::isPacketForwarded(cMessage * msg)
        {
            ...
            for (std::vector<LoRaAppPacket>::iterator lbptr =
                 ...
                 lbptr++)
            {
                if (packet->getMsgType() == lbptr->getMsgType() && packet->getDataInt() == lbptr->getDataInt() && packet->getSource() == lbptr->getSource() && packet->getDestination() == lbptr->getDestination())
                {
                    return true;
                }
            }
            ...
            if (singleMetricRoutingTable.size() > 0)
            {
                ...
                do
                {
                    ...
                    for (std::vector<singleMetricRoute>::iterator smr =
                             singleMetricRoutingTable.begin();
                         smr < singleMetricRoutingTable.end();
                         smr++)
                    {
            ...
            else if (dualMetricRoutingTable.size() > 0)
            {
                ...
                do
                {
                    ...
                    for (std::vector<dualMetricRoute>::iterator dmr =
                             dualMetricRoutingTable.begin();
                         dmr < dualMetricRoutingTable.end();
                         dmr++)
                    {
